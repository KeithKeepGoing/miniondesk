...
- Append a brief summary to {{DATA_DIR}}/MEMORY.md (create if it doesn't exist).
...
2. 進度日誌: Create {{DATA_DIR}}/progress.log and write each key step with timestamp.
...
### 自我認知 (Self-Identity)
Your identity persists across sessions via the `## 身份 (Identity)` section in {{DATA_DIR}}/MEMORY.md.
