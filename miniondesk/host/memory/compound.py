...
async def run_weekly_compound(jid: str) -> None:
    """Prune old warm logs and distill patterns to hot memory."""
    try:
        ...
        log.info("compound: done for jid=%s (pruned %d)", jid, pruned)
    except Exception as exc:
        log.error("compound: failed for jid=%s: %s", jid, exc)
