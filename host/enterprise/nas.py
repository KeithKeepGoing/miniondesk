"""
NAS Deep API Integration for MinionDesk.
Supports NetApp ONTAP REST API and IBM Spectrum Scale (GPFS) REST API.
...
# Config
NETAPP_URL = os.getenv("NETAPP_URL", "").rstrip("/")  # https://netapp.corp.local
NETAPP_SVM = os.getenv("NETAPP_SVM", "")             # Storage Virtual Machine name

GPFS_URL = os.getenv("GPFS_URL", "").rstrip("/")     # https://gpfs.corp.local:443
GPFS_USER = os.getenv("GPFS_USER", "")
GPFS_FILESYSTEM = os.getenv("GPFS_FILESYSTEM", "")   # e.g. fs1
...
def _validate_api_url(name: str, url: str) -> bool:
    ...
    try:
        ...
        return False
    if p.scheme != "https":
        log.error("NAS config: %s must use https:// (got %r)", name, p.scheme)
        return False
    if not p.netloc:
...
_GPFS_FS_QUOTED = urllib.parse.quote(GPFS_FILESYSTEM, safe="") if GPFS_FILESYSTEM else ""
...
def _gpfs_headers() -> dict:
    user = os.getenv("GPFS_USER", "")
    password = os.getenv("GPFS_PASSWORD", "")
    if user:
        import base64
        creds = base64.b64encode(f"{user}:{password}".encode()).decode()
        return {"Authorization": f"Basic {creds}", "Accept": "application/json"}
    return {"Authorization": f"Bearer {password}", "Accept": "application/json"}
...
def _validate_nas_name(val: str) -> str:
    ...
    if val and re.fullmatch(r'\.+', val):
        raise ValueError(f"Invalid NAS name (dot-only names not allowed): {val!r}")
    return val


# ─── NetApp ONTAP ──────────────────────────────────────────────────────────────

def netapp_get_volumes() -> list[dict]:
    """Get all volumes with usage stats from NetApp ONTAP."""
    if not NETAPP_URL:
        return []
...
def netapp_get_volume_usage(volume_name: str) -> Optional[dict]:
    """Get detailed usage for a specific NetApp volume."""
    if not NETAPP_URL:
        return None
...
def netapp_list_snapshots(volume_name: str) -> list[dict]:
    """List snapshots for a NetApp volume."""
    if not NETAPP_URL:
        return []
    # First get volume UUID
    vol = netapp_get_volume_usage(volume_name)
    if not vol:
        return []
...
def netapp_quota_report(volume_name: str = "") -> list[dict]:
    """Get quota report for NetApp volumes."""
    if not NETAPP_URL:
        return []
...
def gpfs_get_filesystem_usage() -> Optional[dict]:
    """Get IBM GPFS filesystem usage."""
    if not GPFS_URL or not GPFS_FILESYSTEM:
        return None
    url = f"{GPFS_URL}/scalemgmt/v2/filesystems/{_GPFS_FS_QUOTED}"
    return _http_get(url, _gpfs_headers())


def gpfs_get_quotas(fileset: str = "") -> list[dict]:
    """Get GPFS quota information."""
    if not GPFS_URL or not GPFS_FILESYSTEM:
        return []
    try:
        if fileset:
        ...
        log.error(f"Invalid fileset name: {e}")
        return []
    url = f"{GPFS_URL}/scalemgmt/v2/filesystems/{_GPFS_FS_QUOTED}/quotas"
    if fileset:
        filter_value = f"filesystemName={_GPFS_FS_QUOTED},filesetName={urllib.parse.quote(fileset, safe='')}"
...
def gpfs_list_snapshots(fileset: str = "") -> list[dict]:
    """List GPFS snapshots."""
    if not GPFS_URL or not GPFS_FILESYSTEM:
        return []
    try:
        ...
        log.error(f"Invalid fileset name: {e}")
        return []
    base = f"{GPFS_URL}/scalemgmt/v2/filesystems/{_GPFS_FS_QUOTED}/snapshots"
    if fileset:
...
# ─── Unified NAS Functions ────────────────────────────────────────────────────

def get_storage_overview() -> str:
    """Get storage overview from available NAS systems."""
    results = []

    if NETAPP_URL:
        volumes = netapp_get_volumes()
        if volumes:
    ...
    if GPFS_URL and GPFS_FILESYSTEM:
        fs = gpfs_get_filesystem_usage()
        if fs and "filesystems" in str(fs):
            results.append(f"\n*📦 IBM GPFS ({GPFS_FILESYSTEM}) 狀態：*")
            results.append(str(fs)[:300])
...
def get_snapshot_restore_guide(volume: str, filename: str = "") -> str:
    """
    Provide step-by-step snapshot restore guidance.
    """
    try:
    ...
    if NETAPP_URL:
        snapshots = netapp_list_snapshots(volume)
        if snapshots:
...
# ─── Tool Functions for Container ─────────────────────────────────────────────

def query_nas_deep(args: dict, ctx) -> str:
    ...
    if action == "overview":
        ...
        return get_snapshot_restore_guide(volume, filename)
    elif action == "quota":
        if NETAPP_URL and volume:
            quotas = netapp_quota_report(volume)
            if quotas:
                ...
                return "\n".join(lines)
        if GPFS_URL:
            quotas = gpfs_get_quotas(fileset)
            if quotas:
